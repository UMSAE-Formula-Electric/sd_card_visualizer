...
function LineChart({data}) {
    ...
    const updateLine = (input, color, yRange,translation,myLine, _class) => {
	...
	*/
		...
		createScale(svg, xsInView, color, yRange, translation, myLine, _class)
			
	}
	const createScale = (svg, xsInView, color, yRange, translation, myLine, _class) => {	
...
/* 		const myLine2 = new line()
	...
	/* 	svg.append("g")
		...
		svg
			//.append("path")
			.selectAll("." + _class)//myLine.line,path)
			.data([xsInView])
			.join("path")//,
				//enter => enter.append(),
				//update => update,
...
/* 			svg
	...
	let myLine = new line()
	let myLine2 = new line()
	
	updateLine(xs, "blue", 150, 0, myLine, "one"); // Redraw chart with new metrics
	updateLine(xsSpeed, "red", 50, -40, myLine2, "two"); // Redraw chart with new metrics
    ...
    function updateGraph() {
		...
		*/
		...
		const speedPoint = {
			x: time, 
			y: Math.random() * 30,//adds value to y component
		};
